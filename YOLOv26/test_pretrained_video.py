...
# Output video path (distinguished from the other script's output)
OUTPUT_VIDEO_PATH = os.path.join(SCRIPT_DIR, "runs", "inference_pretrained_output.mp4")
...
def recognize_action(player_box, ball_box, previous_ball_y):
    # Action recognition logic from test_video.py
    if ball_box is None:
        return "standing"
    ...
    while cap.isOpened():
        ...
        if len(results) > 0:
            ...
            for box in boxes:
                ...
                # NOTE: For a typical pretrained COCO model (like yolo26n.pt might be), 
                # 'person' is class 0, and 'sports ball' is class 32. 
                # This is adapted from the trained model logic (where ball was 0, players were 1, 2)
                if cls_id == 32:  
            ...
            for (p_box, cls_id) in player_boxes:
                # Pretrained COCO model will only detect generic 'person' (not team 1 vs 2)
                color = (255, 0, 0) # Blue color for all persons detected
